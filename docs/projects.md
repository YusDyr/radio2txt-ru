...
## Сравнение экосистем: США vs Россия

| Аспект | США | РФ |
|--------|-----|-----|
| **Стандарт связи** | P25 (цифровой, транковый) | Аналоговое FM (148–174 МГц) |
| **Декодирование** | trunk-recorder / SDRTrunk (зрелые) | rtl_fm (примитивное) |
| **Шифрование** | Растёт, но много открытого | Нет (аналог не шифруется) |
| **Готовые ASR** | English Whisper из коробки, fine-tuned модели | Нет специализированных для радиоэфира |
| **Датасеты** | Broadcastify, OpenMHz — терабайты записей | Ничего публичного |
| **Легальность** | Законно слушать (1st Amendment / public safety) | Серая зона (ФЗ-126) |
| **Сообщество** | Десятки активных проектов, тысячи пользователей | Отсутствует |

### Что можно позаимствовать

1. **Архитектуру trunk-transcribe** — Celery + GPU worker + полнотекстовый поиск. Пайплайн переносится 1:1, заменяется только ASR-модель и язык
2. **Подход imbe-asr** — если в РФ появится цифровое радио (TETRA/DMR), можно обучать ASR напрямую на кодеке без реконструкции аудио
3. **Scanner-map** — извлечение адресов из транскриптов + отображение на карте. Применимо к любому языку с адаптацией NLP
4. **OpenScanner** — уже поддерживает русский через multilingual Whisper, можно использовать как есть для прототипа
